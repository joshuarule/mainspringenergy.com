...
export default ({ location }) => {
  ...
  const [techHeroLoaded, setTechHeroLoaded] = useState(false)
  ...
  const techImageLoaded = img => {
    ...
    if (
      imagesLoaded.current.every(elem => ['first', 'second'].indexOf(elem) > -1)
    ) {
      setTechHeroLoaded(true)
    }
  }
  ...
  return (
    <Layout navTheme="dark" location={location}>
      ...
      <div className="hero  relative">
        ...
        {/* noise */}
        <div
          className="absolute left-0 right-0 top-0 h-screen opacity-50"
          style={{
            backgroundImage:
        ...
        <div className="absolute w-full" style={{ bottom: '-1px' }}>
          <Img
            ...
            className={`techHero-image ${
              techHeroLoaded ? 'techHero-image--loaded' : ''
            }`}
            fadeIn={false}
            onLoad={() => techImageLoaded('first')}
          />
        </div>
      </div>
      <Img
        fluid={heroBottom.childImageSharp.fluid}
        fadeIn={false}
        className={`techHero-image ${
          techHeroLoaded ? 'techHero-image--loaded' : ''
        }`}
